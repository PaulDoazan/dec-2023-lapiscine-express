...
const createCoworkingWithImage = (req, res) => {
    User.findOne({ where: { username: req.username } })
        .then(user => {
            if (!user) {
                return res.status(404).json({ message: `L'utilisateur n'a pas été trouvé.` })
...
const updateCoworking = (req, res) => {
    Coworking.findByPk(req.params.id)
        .then((result) => {
            if (result) {
                return result.update({ ...req.body, picture: `${req.protocol}://${req.get('host')}/images/${req.file.filename}` })
                    .then(() => {
...
const updateCoworkingWithImage = (req, res) => {
    Coworking.findByPk(req.params.id)
        .then((result) => {
            if (result) {
...
module.exports = { findAllCoworkings, findCoworkingByPk, createCoworking, updateCoworking, deleteCoworking, createCoworkingWithImage, updateCoworkingWithImage }